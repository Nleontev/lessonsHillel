...
function Slider({ wrapperId, itemsId, prevId, nextId, paginationId, slides }) {
	...
	this.itemsEl.addEventListener("mousemove", (e) => {
		...
		diffX = startDrawPosX - e.clientX;
		if(!((this.currentIndex === 0 && diffX < 0) || (this.currentIndex === this.slidesLength - 1 && diffX > 0))){
			this.itemsEl.style.transform = `translate(${-(diffX + (wrapperWidth * this.currentIndex))}px, 0)`;
		}
	...
	this.itemsEl.addEventListener("mouseup", (e) => {
		if (!isDrawing) return;
		if(diffX < leftThreshold || diffX >rightThreshold){
